...
st="/mnt/disk1/GB/Erate/results/Al_GB_5_19_404_erate.dump"

node = import_file(st, multiple_frames = True)
#node = import_file(st, multiple_frames = False)

g = open('a_segmentMigration_5_19_404_erate.txt', 'w')
...
for f in range(f_start, ovito.dataset.anim.last_frame+1):
    ...
    cotangens_Alpha = 100 / z_length_sloya 
    ...
    for segment in network.segments:
        ...
        for point in segment.points:
            ...
            if x_end != 0 or y_end != 0 or z_end != 0:
               ...
               for i in range(0, number_layer, 1):
                  #добавляем смещение изза сдвиговой деформации
                  x_fix1 = z_end * cotangens_Alpha
                  x_fix2 = z_beg * cotangens_Alpha
                  ...
                  if x_end > (i*x_length_sloya) + x_fix1 and x_end < (i+1)*x_length_sloya + x_fix1 and x_beg> i*x_length_sloya + x_fix2 and x_beg < (i+1)*x_length_sloya + x_fix2:
                     ...
                     koncentraciya[i] += dlina_megdu_tochkami
                  #иначе находим в каких слоях находятся точки 
                     ...
                     for j in range (0, number_layer, 1):
                        if x_beg> j*x_length_sloya + x_fix2 and x_beg < (j+1)*x_length_sloya + x_fix2:
                           n = j - i  #kolichestvo ploskostey megdu tochkami
                           ...
                           if n < 0: # koordinata ploskosti sleva
                              ...
                              for l in range(i, j-1, -1):
                                 ...
                                 x_plane1 = l*x_length_sloya + z_plane1 * cotangens_Alpha
                                 x_plane2 = l*x_length_sloya + z_plane2 * cotangens_Alpha
                                 x_plane3 = l*x_length_sloya + z_plane3 * cotangens_Alpha
                              ...
                              for l in range(i, j+1, 1):
                                 ...
                                 x_plane1 = (l+1)*x_length_sloya + z_plane1 * cotangens_Alpha
                                 x_plane2 = (l+1)*x_length_sloya + z_plane2 * cotangens_Alpha
                                 x_plane3 = (l+1)*x_length_sloya + z_plane3 * cotangens_Alpha
                                 #вычисляем длину вектора вплоть до пересечения ее с плоскостью
                                 dlina_vectora = function_peresecheniya(x_end, y_end, z_end, x_beg, y_beg, z_beg, x_plane1, y_plane1, z_plane1, x_plane2, y_plane2, z_plane2, x_plane3, y_plane3, z_plane3)  
                                 #записываем эту часть длины вектора в определенный слой
                                 koncentraciya[l] = koncentraciya[l] + (dlina_vectora - dlina_ostatochnoy_chasty)
    ...
    g.write("%i %f %f %f %f %f %f %f %f %f %f %f \n" % (f, time, koncentraciya[0], koncentraciya[1], koncentraciya[2], koncentraciya[3], koncentraciya[4], koncentraciya[5], koncentraciya[6], koncentraciya[7], koncentraciya[8], koncentraciya[9]))
